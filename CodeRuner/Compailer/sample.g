...
forloop returns[string Csharp]:
								FOR{$Csharp+="for (";} 
								(a=ID{$Csharp+=$a.text+"=0";}|aa=tarife_moteghir{$Csharp+=$tarife_moteghir.Csharp;}|entesab {$Csharp=$entesab.Csharp;})
								{$Csharp+=";";} TO b=ebarat 
								{
									$Csharp+=$a.text|aa.id +"<"+ $b.Csharp+";"+$a.text|aa.id+"++)"+Environment.NewLine;
								}
								THEN BEGIN {$Csharp+=" {";}
...
noe returns [string Type,string Csharp] 
							...
							| BOOL {$Csharp="bool";$Type = "dool"; }
							| BOOL {$Csharp="double";$Type = "double"; }
							| BOOL {$Csharp="byte";$Type = "byte"; };
